...
class PressureChanging(AbstractModulePreFe):

    def run(self, material_workpiece_supplier, material_workpiece_name, material_tool_supplier, material_tool_name, lubricants_lubricant_supplier, lubricants_lubricant_name, pressureconditions_pmin, pressureconditions_pmax, pressureconditions_type, pressureconditions_rate, otherpressureconditions_t, otherpressureconditions_v, create_file=True, target_directory=None, **kwargs):
...
# if __name__ == "__main__":
        ...
        #User input
        Pmin = pressureconditions_pmin #MPa
        Pmax = pressureconditions_pmax
        ...
        P_jump_point_array = np.linspace(0,75,numbcurves)
        changing_type = pressureconditions_type
        
        if changing_type == "pressure increase":
            P1 = Pmin/1000
            P2 = Pmax/1000
        else:
            P1 = Pmax/1000
        ...
        T = otherpressureconditions_t # C
        T = T + 273.1
        v = otherpressureconditions_v #mm/s
...
## For multi curve: Temperature, Pressure and Sliding velocity or for I.f interactive figure calculation
        ...
        ## For multi curve: Temperature, Pressure and Sliding velocit
            ...
            P_jump_step = round(P_jump_point_array[i]/(v*t_step))

            timesteps = round(max_sliding_distance/v/t_step) #changed from SD to max SD (constant)
        ...
        # initial lubricant thickness: um
            ...
            for j in range(P_jump_step):
                ...
                ht_dot[j]=-ht[j]*(c*(P1**k1)*(v**k2)/(ita**k3))
                ht[j+1]=ht[j]+ht_dot[j]*t_step
                beta[j]=np.exp(-(lambda1*ht[j])**lambda2)
                u[j]=(1-beta[j])*ul+beta[j]*ud
                
            for j in range(P_jump_step, timesteps):
                t[j]=(j-1)*t_step
                
                sldingdistance_step = t_step * v 

                slidingdistance[j+1] = slidingdistance[j] + sldingdistance_step
                ht_dot[j]=-ht[j]*(c*(P2**k1)*(v**k2)/(ita**k3))
                ht[j+1]=ht[j]+ht_dot[j]*t_step
                beta[j]=np.exp(-(lambda1*ht[j])**lambda2)
                u[j]=(1-beta[j])*ul+beta[j]*ud

            for j in range(timesteps):
                ht_dot_min[j]=-ht_min[j]*(c*((Pmin/1000)**k1)*(v**k2)/(ita_min**k3))
                ...
                ht_dot_max[j]=-ht_max[j]*(c*((Pmax/1000)**k1)*(v**k2)/(ita_max**k3))
    ...
    #         #Label = f'Temp = {T} °C  P = {P*1000} MPa  Vel = {v} mm/s'    
            ...
            if changing_type == "pressure increase":
                fig.add_trace(go.Scatter(visible = False, x=slidingdistance[0:-1], y=u[0:-1],mode='lines', line=dict(width=2),name='COF (Pressure increase)',yaxis = 'y1')) #legendgroup='group1', name = f'Effect of Temperature = {T - 273:,.2f} C' , mode='lines', line=dict(width=2)))
            else:
                fig.add_trace(go.Scatter(visible = False, x=slidingdistance[0:-1], y=u[0:-1],mode='lines', line=dict(width=2),name='COF (Pressure decrease)',yaxis = 'y1')) #legendgroup='group1', name = f'Effect of Temperature = {T - 273:,.2f} C' , mode='lines', line=dict(width=2)))
            if changing_type == "pressure increase":
                fig.add_trace(go.Scatter(visible = False, x=slidingdistance[0:-1], y=P, mode='lines', line=dict(width=2,dash='dashdot'), name='Pressure increase',),  secondary_y=True)
            else:
                fig.add_trace(go.Scatter(visible = False, x=slidingdistance[0:-1], y=P, mode='lines', line=dict(width=2,dash='dashdot'), name='Pressure decrease',),  secondary_y=True)
            
            fig.add_trace(go.Scatter(visible = False, x=slidingdistance[0:-1], y=u_min[0:-1],mode='lines', line=dict(width=2, dash='dash'),name=f'COF for Pmin={Pmin}MPa',yaxis = 'y1'))

            fig.add_trace(go.Scatter(visible = False, x=slidingdistance[0:-1], y=u_max[0:-1],mode='lines', line=dict(width=2, dash='dot'),name=f'COF for Pmax={Pmax}MPa',yaxis = 'y1'))
        ...
        for z in np.arange(0,len(fig.data),4):
            step = dict(
                ...
                label = str(round(P_jump_point_array[int(z/4)],1)) # layout attribute
            )
        ...
        sliders = [dict(
            active=0,
            currentvalue={"prefix": 'Pressure jump point:'},
            pad={"t": 55},
            steps=steps
        )]
        ...
        fig.update_yaxes(title_text="Pressure (MPa)", secondary_y=True)
        # fig.show()
        ...
        return {
                'files': [
                    {'filename': 'COF.csv', 'data': Avg_COF_formatted}
                ],
                'figures': [
                    {'figurename': 'contact_pressure_figure', 'figure': fig_asdict}
                ]
            }
