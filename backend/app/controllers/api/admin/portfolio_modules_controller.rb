...
module Api
  module Admin
    class PortfolioModulesController < Api::Admin::BaseController
      include AutoInject[
        create_module_validation: 'validations.portfolio_modules.admins.create_validation',
        create_admin_module: 'services.portfolio_modules.admins.create',
        filter: 'queries.portfolio_modules.filter',
        update_module_validation: 'validations.portfolio_modules.admins.update_validation',
        update_module: 'services.portfolio_modules.admins.update',
      ...
      def create
        if create_validation.success?
          record = create_admin_module.(portfolio_module_params)
          render json: serializer.new(
      ...
      def prepare_portfolio_cm_attributes                 
        ...
        attributes = (cm[:pre_fe] + cm[:post_fe]).compact.uniq.each_with_index.inject([]) do |all, (id, index)|
          all << {
            computation_module_id: id.to_i, sort_index: index
          }
        end
      ...
      def portfolio_module
        @portfolio_module ||= PortfolioModule.find(params[:id])
      end

      def portfolio_modules
        @portfolio_modules ||= filter.(filter_params)
      end
      ...
      def create_validation
        create_module_validation.(
          portfolio_module_params
        )
      end

      def update_validation
        update_module_validation.with(record: portfolio_module).(portfolio_module_params)
      end
    end
  end
end
