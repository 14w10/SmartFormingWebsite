...
type DeclineFormProps = {
  handleClose: () => void;
  handleChangeStatus: (newStatus: any, data?: { declineReason: string }) => void;
};
...
export const DeclineForm = ({ handleClose, handleChangeStatus }: DeclineFormProps) => {
  ...
  const onSubmit = useCallback(
    ({ declineReason }: { declineReason: string }) => {
      handleChangeStatus({ status: 'declined', declineReason, portfolioId: query.portfolioId });
      handleClose();
    },
    [handleChangeStatus, handleClose, query.portfolioId],
  );
